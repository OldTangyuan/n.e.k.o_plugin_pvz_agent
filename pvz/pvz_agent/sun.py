...
- **中心发光判别**：阳光是发光的金色球体，中心有强高光（亮度 V 很高，贴图≈255）；
  文字/UI 图标中心是笔画或暗色，中心不发光。即使笔画里恰好有零星深黄像素
  （占比落在 3%~55% 的判定窗口），中心不发光也会被剔除（与中心深黄表情特征组合）。
...
class SunCollector:
    ...
    def _has_core_texture(self, hsv: np.ndarray, cx: float, cy: float, radius: float) -> bool:
        """检查候选中心是否具备"阳光核心"特征：深黄表情 + 中心发光。

        真实阳光 = 低饱和奶油黄圆盘 + 中心一块高饱和深黄表情（眼睛/嘴）+ 中心高光。
        - 深黄表情色相非常集中：H 24~32（纯黄），S≥200（实测真阳光 ≈12.7%）；
        - 中心发光：阳光是发光球体，中心圆平均亮度 V 很高（贴图 ≈255）。

        文字/UI 干扰：即使笔画里恰好有零星深黄像素（占比落在 3%~55%），
        中心是暗笔画或背景、不发光（V 明显低于阳光），也会被剔除。

        判定：中心圆（0.55×R）内 纯黄高饱和占比在 3%~55% 之间 **且** 中心平均 V≥200。
        """
        if radius < 4.0:
        ...
        for yy in range(y0, y1):
            for xx in range(x0, x1):
                if (xx - cx_i) ** 2 + (yy - cy_i) ** 2 <= r_px * r_px:
                    total += 1
                    H, S, V = int(hsv[yy, xx, 0]), int(hsv[yy, xx, 1]), int(hsv[yy, xx, 2])
                    v_sum += V
        ...
        center_v = v_sum / total
        # 中心发光（阳光固有，贴图 V≈255）；文字笔画中心偏暗 → 剔除
        if center_v < 200:
            return False
