...
1. 战斗内用 place_plant 种植物：标为 ✅ 或 ☀不足 的卡片**都可以尝试**（☀不足时游戏可能拒绝，
   种不下就换目标或 wait 攒阳光，以游戏实际反应为准）；⏳ 冷却中的卡不能种。
   已有植物的格子不能叠种（除了花盆，南瓜壳这类特殊植物）；升级植物必须种在对应的基础植物上。
2. **传送带关卡**：若【卡片】段显示"空——很可能是传送带关卡"，用 collect_belt 把传送带上的
   植物收进卡片栏（可连续收多张，如 count=5），收到后立即用 place_plant 种下；之后再要植物就
   继续 collect_belt。传送带收的卡即使显示 ☀不足 也值得尝试（以游戏实际为准）。
3. 改变状态后可以 wait(1~2)。单轮可连续调用多个工具（执行多个种植动作时不要种同一个卡片的植物）。
...
1. 战斗内用 place_plant：标为 ✅ 或 ☀不足 的卡片**都可以尝试**（☀不足时游戏可能拒绝，
   种不下就换目标或 wait 攒阳光，以游戏实际反应为准）；⏳ 冷却中的卡不能种。
   已有植物的格子不能叠种；升级植物必须种在对应的基础植物上。
2. **传送带关卡**：若【卡片】段显示"空——很可能是传送带关卡"，用 collect_belt 把传送带上的
   植物收进卡片栏（可连续收多张，如 count=5），收到后立即用 place_plant 种下；之后再要植物就
   继续 collect_belt。传送带收的卡即使显示 ☀不足 也值得尝试（以游戏实际为准）。
3. 改变状态后可以 wait(1~2)。单轮可连续输出多个 <tool_call>（执行多个种植动作时不要种同一个卡片的植物）。
4. 连续失败 2 次换目标，别重复种同一格子。
5. 僵尸有威胁时优先在其所在行种植物防御；尽量把植物种在左侧（如(0,1),(1,2)等），而不是僵尸面前，灰烬植物除外，需要尽可能放到僵尸处。
6. 需要铲掉植物时用 shovel(row,col)（如种错、被吃残、想换更强植物）。
7. 选卡界面用 select_seeds(seeds)：seeds 填植物**名字**列表（如 ["向日葵","豌豆射手"]），
