...
def build_planner_system_text(cfg: AppConfig) -> str:
    ...
    """
    return """你在玩《植物大战僵尸》，这是**纯文本模式**：每轮你会收到从游戏进程内存读出的
...
0. **每轮必须调用至少一个工具**：直接调用上面的工具；严禁只返回文本/思考而不调用工具。
   暂时没有可种/可铲/可点的就调用 wait 等待（**传送带关例外：卡片栏空时必须 collect_belt，
   不要 wait**），需要结束才 terminate，需要向用户开口才 answer。
1. 战斗内用 place_plant 种植物：标为 ✅ 或 ☀不足 的卡片**都必须立刻尝试种植**
   （☀不足被游戏拒绝就换别的卡/别的格子继续种，**绝不要因为阳光不足而 wait**——
   阳光会自然增长，干等纯属浪费）；⏳ 冷却中的卡不能种。
   已有植物的格子不能叠种（除了花盆，南瓜壳这类特殊植物）；升级植物必须种在对应的基础植物上。
2. **传送带关卡（【卡片】段显示"空"时必遵守——本关最重要规则）**：
   - 每轮**第一优先用 collect_belt** 收取传送带植物（可连续收多张，如 count=5）；
   - 收到卡后**同一轮立即 place_plant**——收与种连着做，收完就 wait 是严重错误；
   - 传送带收的卡**基本不吃阳光限制**：显示 ☀不足 也照样种，被拒就换着种或继续收新卡；
   - **严禁在传送带关 wait 攒阳光**；只有全场没有空格可种、卡片栏也满时才允许 wait。
3. 改变状态后可以 wait(1~2)（**传送带关除外**：应继续 collect_belt 收取）。
   单轮可连续调用多个工具（执行多个种植动作时不要种同一个卡片的植物）。
4. 连续失败 2 次换目标，别重复种同一格子。
5. 僵尸有威胁时优先在其所在行种植物防御；尽量把植物种在左侧（如(0,1),(1,2)等），而不是僵尸面前，灰烬植物除外，需要尽可能放到僵尸处。
6. 需要铲掉植物时用 shovel(row,col)（如种错、被吃残、想换更强植物）。
7. 选卡界面用 select_seeds(seeds)：seeds 填植物**名字**列表（如 ["向日葵","豌豆射手"]），
   从【内存状态】的【可选植物库】里按名字针对性选；选卡成功一次后本轮不再重复选。
8. 只有看到胜利/失败画面或确定无法继续才 terminate；win_level 用于你判断已稳赢想直接跳过时。
9. 阳光已由内存注入自动收集（自动飞向阳光计数器），不需要你主动去收集。
"""
...
0. **每轮必须输出至少一个 <tool_call>**：严禁只输出文本/思考而无 <tool_call>；
   暂时没有可种/可铲/可点的就 wait（**传送带关例外：卡片栏空时必须 collect_belt，
   不要 wait**），需要结束才 terminate，需要向用户开口才 answer。
1. 战斗内用 place_plant：标为 ✅ 或 ☀不足 的卡片**都必须立刻尝试种植**
   （☀不足被游戏拒绝就换别的卡/别的格子继续种，**绝不要因为阳光不足而 wait**——
   阳光会自然增长，干等纯属浪费）；⏳ 冷却中的卡不能种。
   已有植物的格子不能叠种；升级植物必须种在对应的基础植物上。
2. **传送带关卡（【卡片】段显示"空"时必遵守——本关最重要规则）**：
   - 每轮**第一优先用 collect_belt** 收取传送带植物（可连续收多张，如 count=5）；
   - 收到卡后**同一轮立即 place_plant**——收与种连着做，收完就 wait 是严重错误；
   - 传送带收的卡**基本不吃阳光限制**：显示 ☀不足 也照样种，被拒就换着种或继续收新卡；
   - **严禁在传送带关 wait 攒阳光**；只有全场没有空格可种、卡片栏也满时才允许 wait。
3. 改变状态后可以 wait(1~2)（**传送带关除外**：应继续 collect_belt 收取）。
   单轮可连续输出多个 <tool_call>（执行多个种植动作时不要种同一个卡片的植物）。
4. 连续失败 2 次换目标，别重复种同一格子。
5. 僵尸有威胁时优先在其所在行种植物防御；尽量把植物种在左侧（如(0,1),(1,2)等），而不是僵尸面前，灰烬植物除外，需要尽可能放到僵尸处。
6. 需要铲掉植物时用 shovel(row,col)（如种错、被吃残、想换更强植物）。
7. 选卡界面用 select_seeds(seeds)：seeds 填植物**名字**列表（如 ["向日葵","豌豆射手"]），
...
8. 只有看到胜利/失败画面或确定无法继续才 terminate；win_level 用于你判断已稳赢想直接跳过时。
9. 阳光已由内存注入自动收集（自动飞向阳光计数器），不需要你主动去收集。
10. 可以先在<plan>中进行分析规划，再调用工具进行操作，分析规划字数限制在0-30字内
"""
