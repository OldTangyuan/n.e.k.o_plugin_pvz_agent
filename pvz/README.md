...
> 🎯 **版本稳定性**：**`pvz 1.0.0.1051 EN`（原版）最稳定**（读内存/注入/铲除/选卡全支持且验证充分）；
> 其它 8 个受支持版本**可玩**但**读内存可能不稳**（部分偏移"需实测"，写错可能崩）；**杂交版
> 正在适配中**（未支持读内存）。建议纯文本模式用原版。

### 选卡界面（`agent_controls_seed_selection`）

- `false`（默认）：**选卡不触发 LLM**，由玩家手动选卡后进入战斗；
- `true`：选卡界面喂 LLM，模型按【可选植物库】用 `select_seeds` 自动选卡（每关只选一次）。
